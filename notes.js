...
function showNotes() {
    ...
    notesObj.forEach(function (element, index) {
        ...
        html += ` <div class="card noteCard " style="width: 18rem;height:18rem;margin:10px;padding-top:20px;  overflow-y: auto; box-shadow: rgba(0, 0, 0, 0.25) 0px 54px 55px, rgba(0, 0, 0, 0.12) 0px -12px 30px, rgba(0, 0, 0, 0.12) 0px 4px 6px, rgba(0, 0, 0, 0.17) 0px 12px 13px, rgba(0, 0, 0, 0.09) 0px -3px 5px; border-width: 0px;">
        <div class="card-body">
            <h5 class="card-title">${(titleObj[index])}</h5>
            <hr>
            <p class="card-text" id="edit${index}t" style="font-size:1.22rem">${element}</p>
            ...
            <i style="top: 3px; position: absolute; color:blue;" >${index + 1}.</i>
        </div>
    </div> `
    });
...
function editNotes(index) {
    ...
    if (complete == 1) {
        ...
        let html = `<textarea class="form-control mx-2 my-2 textArea" placeholder="Edit Note" id="${index}">${text}</textarea>`
